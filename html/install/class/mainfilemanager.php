...
// 20111020 filename change
    var $path = '../mainfile.php';
    var $distfile = '../mainfile.dist.php';
    var $custom_path = '../settings/definition.custom.php';
    var $custom_distfile = '../settings/definition.custom.dist.php';
    var $rewrite = array();
    ...
    */
        // 20111020 custom add
        if ( ! copy($this->custom_distfile, $this->custom_path) ) {
            $this->report[] = _NGIMG.sprintf(_INSTALL_L126, '<b>'.$this->custom_path.'</b>');
            $this->error = true;
            return false;
        }
//        $this->report[] = _OKIMG.sprintf(_INSTALL_L125, '<b>'.$this->path.'</b>', '<b>'.$this->distfile.'</b>');
        $this->report[] = _OKIMG.sprintf(_INSTALL_L125, '<b>'.$this->custom_path.'</b>', '<b>'.$this->custom_distfile.'</b>');
        return true;
    }

    function doRewrite(){
        if ( ! $file = fopen($this->custom_path,"r") ) {
            $this->error = true;
            return false;
        }
        clearstatcache();
        $content = fread($file, filesize($this->custom_path) );
        fclose($file);
        ...
        if ( !$file = fopen($this->custom_path,"w") ) {
            $this->error = true;
            return false;
        }

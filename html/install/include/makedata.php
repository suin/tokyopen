...
function make_data(&$dbm, &$cm, $adminname, $adminpass, $adminmail, $language, $gruops){
    ...
    $dbm->insert('config', " VALUES (1, 0, 1, 'sitename', '_MD_AM_SITENAME', 'TOKYOPen Site', '_MD_AM_SITENAMEDSC', 'textbox', 'text', 0)");
    ...
    $temp = md5($adminpass);
    $regdate = time();
    $dbm->insert('users', " VALUES (1,'','".addslashes($adminname)."','".addslashes($adminmail)."','".XOOPS_URL."/','blank.gif','".$regdate."','','','',1,'','','','','".$temp."',0,0,7,5,'','".$time_diff."',".time().",'thread',0,1,0,'','','',0)");

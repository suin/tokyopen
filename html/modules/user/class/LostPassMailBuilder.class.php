...
class User_LostPass1MailBuilder
{
	...
	function User_LostPass1MailBuilder(array &$mConfig)
	{
	...
	function setBody($user,$xoopsConfig,$extraVars)
	{
		...
		$this->mMailer->assign("NEWPWD_LINK", XOOPS_URL . "/lostpass.php?try_field=" . $user->getShow($this->mConfig['lost_pass_method']) . "&code=" . substr($user->get("pass"), 0, 5));
	}

			...
			if ($_data[0]){
				...
				if ($user){
					for ($i=0; $i<count($user_key); $i++){
						$csv_value = isset($_data[$i]) ? $_data[$i] : '';
				...
				for ($i=0; $i<count($user_key); $i++){
					...
					switch ($user_key[$i]){
                        case 'user_regdate':
                            if (! isset($var) || $var === '') {
                                $var = time();
                            } else if ($var === $user_tmp->get('user_regdate')) {
                                $var = date('Y/n/j H:i', $var);
                            }
                            $var = formatTimestamp(strtotime($var), 'Y/n/j H:i');
                            break;
                        case 'last_login':
					    ...
					    case 'timezone_offset':
					        $var = $GLOBALS['xoopsConfig']['default_TZ'];
					        break;
					    case 'umode':
					        $var = $GLOBALS['xoopsConfig']['com_mode'];
					        break;
					    case 'uorder':
					        $var = $GLOBALS['xoopsConfig']['com_order'];
					        break;
					}
					$user_data['value'][] = array(
						'var'    => $var,

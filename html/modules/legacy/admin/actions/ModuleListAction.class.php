...
class Legacy_ModuleListAction extends Legacy_Action
{
        ...
        foreach($t_objectArr as $module) {
            ...
            if (count(array_diff_assoc($olddata, $newdata)) > 0 ) {
                ...
                if ($moduleHandler->insert($module)) {
                	...
                	$this->_notifyUpdateToSiteNavi($module->get('mid'), $module->get('name'));
                }
                else {
                	$successFlag = false;
                }
	...
	protected function _notifyUpdateToSiteNavi($moduleId, $moduleName)
	{
		...
		$isSuccess = $page->updateModuleTitle($moduleId, $moduleName);
		return $isSuccess;
	}
}

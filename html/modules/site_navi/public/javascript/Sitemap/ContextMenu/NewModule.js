 ...
 */
var SiteNavi_Sitemap_ContextMenu_NewModule = function(app) {
	this.app = app;
};

SiteNavi_Sitemap_ContextMenu_NewModule.prototype = {
	 ...
	 */
	outlet: {
		form: '#SiteNavi_Form_AdminModuleEdit',
		formSubmitButton: '#SiteNavi_Form_AdminModuleEdit input[type=submit]'
	},
	 ...
	 * @constant
	 */
	FORM_URL: '/index.php?controller=module_edit&ajaxmode=1',
	...
	setUpContext: function(contextMenu, data) {
		if ( data.type_name != 'page' ) {
			contextMenu.disable('newModule');
		}
	},
	...
	_registerEventListeners: function(contextMenu) {
		...
		contextMenu.delegate('newModule', function(data){ self._clickMenuEventHandler(data); });
	},

	/**
	 * コンテクストメニューの「新規ページを追加」をクリックしたとき
	 * @protected
	 * @param {Object} data
	 */

...
class XoopsModule extends XoopsObject
{
    ...
    function getNiceIconUrl()
    {
        $dirname = $this->get('dirname');
        $defaultIcon = XOOPS_URL .'/modules/nice_admin/public/images/default_nice_icon.png';
        $iconUrl = $defaultIcon;

        // check xoops_version.php
        $this->loadInfo($dirname);
        if ( isset($this->modinfo['nice_image']) ){
            $niceImage = sprintf('/modules/%s/%s', $dirname, $this->modinfo['nice_image']);
            $iconUrl = XOOPS_URL . $niceImage;
        }else{
            // check file
            $niceImage = sprintf('/modules/%s/images/module_nice_icon.png', $dirname);
            if ( file_exists(XOOPS_ROOT_PATH.$niceImage) ){
                $iconUrl = XOOPS_URL.$niceImage;
            }
        }

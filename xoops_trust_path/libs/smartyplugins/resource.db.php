...
function smarty_resource_db_tplinfo( $tpl_name )
{
	...
	// 2nd, check templates under ryus_templates_set (file template)
	$filepath = RYUS_TEMPLATES_SET_PATH . '/' . $tpl_name ;
	...
	// 3rd, check templates under ryus_templates_set/(trust based template)
	if( $mytrustdirname && $base_tpl_name ) {
		$filepath = RYUS_TEMPLATES_SET_PATH . '/' . $mytrustdirname . '/' . $base_tpl_name ;
		if ( file_exists( $filepath ) ) {
			return $cache[$tpl_name] = $filepath ;
		}
	}

	// 4th, check templates under themes/(theme)/templates/ (file template)
	$filepath = XOOPS_THEME_PATH . '/' . $theme . '/templates/' . $tpl_name ;
	if ( file_exists( $filepath ) ) {
		return $cache[$tpl_name] = $filepath ;
	}

	// 5th, check templates under themes/(theme)/templates/(trust based template)
	if( $mytrustdirname && $base_tpl_name ) {
	...
	// 6th, find a DB template of the selected tplset
	$tplfile_handler =& xoops_gethandler('tplfile');
	$tplobj = $tplfile_handler->find( $tplset, null, null, null, $tpl_name, true);
	if ( empty( $tplobj ) ) {
		// 7th, find a DB template in default tplset
